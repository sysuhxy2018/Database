...
* char和varchar的区别：
  ...
  * 在update操作时，varchar可能会让行变长（因为varchar按实际字符长度存储，如果update后字符变长了，所占的空间自然也长了），可能会超出存储页的大小。对于MyISAM，由于页上存的只是指针，所以行的变化无所谓，不过仍需要将行拆分成不同片段；对于InnoDB，由于行数据就存储在页上，所以需要分裂页。
